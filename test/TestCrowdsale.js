...
contract('SimpleCrowdsale', function(accounts) {
    it('should deploy the token and store the address', function(done){
        SimpleCrowdsale.deployed().then(async function(instance) {
    ...
    it('one ETH should buy 5 Simple Tokens in PreICO', function(done){
        SimpleCrowdsale.deployed().then(async function(instance) {
            const data = await instance.sendTransaction({ from: accounts[7], value: web3.toWei(1, "ether")});
            const tokenAddress = await instance.token.call();
            const simpleToken = SimpleToken.at(tokenAddress);
            const tokenAmount = await simpleToken.balanceOf(accounts[7]);
    ...
    it('one ETH should buy 2 Simple Tokens in ICO', function(done){
        SimpleCrowdsale.deployed().then(async function(instance) {
